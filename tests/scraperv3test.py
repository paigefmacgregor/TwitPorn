...
try:
    ...
    # ask user for an end date
    date_string2 = raw_input("input ending date in mm/dd/yyyy format: ");
    date_list2 = date_string2.split('/')
    end_date = datetime2(int(date_list[2]), int(date_list[0]), int(date_list[1]))
    print "end date:", end_date
    
    # ask user to confirm the date is correct
    confirm = raw_input("that's " + str((end_date() - start_date).days) + " days of tweets, are you sure? (y/n): ")
    if confirm != 'y':
        exit()
    ...
    for name in usernames:
        ...
        while not is_done:
            ...
            for tweet in tweets:
                # see if tweet date falls before end date...
                end_date_diff = tweet.created_at - end_date
                if end_date_diff.days < 0:
                    # if so, we're done, so break out of loop
                    is_done = True
                    break
                print "tweet date:", tweet.created_at, ",", end_date_diff.days, "days after end date"
